...
import { PrismaAuthorizationRepository } from "../repository/authorization.repository";

export function createAuthorizationDependencies() {
  return {
    repository: new PrismaAuthorizationRepository(),
  };
}

/** The shape every Presentation-layer controller in this module depends on — real deps here, fakes in tests (Ch.10.6). */
export type AuthorizationDependencies = ReturnType<typeof createAuthorizationDependencies>;
