  ...
  return (
    <Card>
      <CardContent className="flex items-start justify-between gap-4 p-5">
        <div>
          <p className="text-sm font-medium text-ink-muted light:text-light-ink-muted">{label}</p>
          <p className="mt-2 text-2xl font-semibold tracking-tight text-ink light:text-light-ink">{value}</p>
          {hint && <p className="mt-1 text-xs text-ink-faint light:text-light-ink-faint">{hint}</p>}
        </div>
